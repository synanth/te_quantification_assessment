...
deg_loc = base_loc + "me2_degs.csv"
vsd_loc = base_loc + "me2_vsd.csv"
metadata_loc = "/lustre/research/dawli/stexocaelum/me_cfs2/metadata.csv"
viz_loc = base_loc + "me2_viz_top_" + top_n + ".png"
raw_loc = "/lustre/research/dawli/stexocaelum/me_cfs2/counts.csv"
...
tes_for_pca = deg_df.sort_values(by="padj")[:int(top_n)].index.tolist()
...
print(deg_df[(abs(deg_df["log2FoldChange"]) >= 1) & (deg_df["pvalue"] <= .01)])
...
fig = make_subplots(rows=3, cols=4, 
    subplot_titles=("PCA of top " + top_n + " TEs based on adjusted <i>P</i>-value", "Volcano plot", te_names[0], te_names[1], te_names[2], te_names[3]),
...
for i in range(0,4):
    ...
    fig.add_trace(go.Box(y=viz_case, jitter=0.5, boxpoints="all", marker={"color" : colors[0]}, name="ME/CFS"), row=3, col=i+1)
...
fig.add_annotation(text="ME/CFS", xref="paper", yref="paper", x=.141, y= 1.065, showarrow=False, font=dict(size=14))
