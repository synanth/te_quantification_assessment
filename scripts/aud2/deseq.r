...
base_loc <- "/lustre/research/dawli/stexocaelum/aud_Backup/"
...
get_upregulated <- function(df){
    key <- intersect(rownames(df)[which(df$log2FoldChange>=1)], rownames(df)[which(df["padj"]<=0.1)])
    results <- key
    return(results)
}            
...
deg_loc <- "/home/stexocae/data_xfer/aud2_degs.csv"
vsd_loc <- "/home/stexocae/data_xfer/aud2_vsd.csv"
