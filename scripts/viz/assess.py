...
name_conversion = {"ervmap": "ERVmap",
                   ...
                   "tetools" : "TEtools",
                   "texp" : "TeXP"}
df_rows = ["ERVmap", "ExplorATE", "LIONS", "SQuIRE", "<b>QuantiTE</b>", "Telescope", "TElocal", "TEtools", "TeXP"]
...
for i,fam in enumerate(["Alu", "ERV", "hAT", "LINE", "MIR", "Other"]):
    ...
    name_loc = [.3135, .4245, .5245, .6245, .7355, .8445]
    ...
    fig.add_annotation(xref='x domain', yref='y domain', x=name_loc[i], y=1.085, text=fam, showarrow=False, row=1, col=1, font=dict(size=14))


fig.update_yaxes(range=[.95,1], showgrid=False, row=1, col=1)
fig.update_xaxes(showticklabels=False, row=1, col=1)

fig.add_annotation(xref="x domain",yref="y domain",x= -.03, y=1.2, showarrow=False, text="<b>A</b>", font=dict(size=24),row=1, col=1)
...
fig.add_annotation(xref='x domain', yref='y domain', x=0.06, y=1.085, text='Precision', showarrow=False, row=1, col=1, font=dict(size=14))
...
fig.add_annotation(xref='x domain', yref='y domain', x=0.1635, y=1.085, text='Sensitivity', showarrow=False, row=1, col=1, font=dict(size=14))
...
for i,fam in enumerate(["Alu", "ERV", "hAT", "LINE", "MIR", "Other"]):
    colors = ["#cd001a", "#ef6a00", "#f2cd00", "#79c300", "#1961ae", "#61007d"]
    dark_colors = ["#66000d", "#773500", "#796600", "#3c6100", "#0c3057", "#30003e"]
    name_loc = [.313, .4235, .5235, .6235, .7335, .8435]
    df_sub = df_family.loc[df_family['family'] == fam]
    method = [name_conversion[x] for x in list(df_sub["method"])]
    sensitivity = list(df_sub["sensitivity"])
    precision = list(df_sub["precision"])
...
fig.add_annotation(xref="x domain",yref="y domain",x= -.073, y=.95, showarrow=False, text="<b>B</b>", font=dict(size=24),row=7, col=1)



fig.add_annotation(xref="x domain",yref="y domain",x=0.5, y=-1.7, showarrow=False,
                   text="Overall sensitivity and precision", font=dict(size=16),row=10, col=1)

fig.add_shape(type="rect", xref='x domain', yref='y domain', x0=0.238, y0=-1, x1=0.254, y1=-1.175, line=dict(color="#74d7ee", width=2), fillcolor="#74d7ee", row=10, col=1)
fig.add_annotation(xref='x domain', yref='y domain', x=0.253, y=-1.29, text='Sensivity', showarrow=False, row=10, col=1, font=dict(size=14))
fig.add_shape(type="rect", xref='x domain', yref='y domain', x0=0.435, y0=-1, x1=0.451, y1=-1.175, line=dict(color="#ffafc8", width=2), fillcolor="#ffafc8", row=10, col=1)
fig.add_annotation(xref='x domain', yref='y domain', x=0.52, y=-1.29, text='Precision', showarrow=False, row=10, col=1, font=dict(size=14))
fig.add_shape(type="rect", xref='x domain', yref='y domain', x0=0.63, y0=-1, x1=0.646, y1=-1.175, line=dict(color="#613915", width=2), fillcolor="#613915", row=10, col=1)
fig.add_annotation(xref='x domain', yref='y domain', x=0.773, y=-1.29, text='F1-score', showarrow=False, row=10, col=1, font=dict(size=14))
...
fig.add_annotation(xref="x domain",yref="y domain",x= -.1, y=.95, showarrow=False, text="<b>C</b>", font=dict(size=24),row=7, col=2)
fig.add_annotation(xref="x domain",yref="y domain",x=0.5, y=-.4, showarrow=False,
                   text="Computational requirements", font=dict(size=16),row=7, col=2)

fig.add_shape(type="rect", xref='x domain', yref='y domain', x0=0.262, y0=-.24, x1=0.278, y1=-.28, line=dict(color="#FFD800", width=2), fillcolor="#FFD800", row=7, col=2)
fig.add_annotation(xref='x domain', yref='y domain', x=0.283, y=-.31, text='Wall clock', showarrow=False, row=7, col=2, font=dict(size=14))
fig.add_shape(type="rect", xref='x domain', yref='y domain', x0=0.452, y0=-.24, x1=0.468, y1=-.28, line=dict(color="#21B1FF", width=2), fillcolor="#21B1FF", row=7, col=2)
fig.add_annotation(xref='x domain', yref='y domain', x=0.546, y=-.31, text='CPU clock', showarrow=False, row=7, col=2, font=dict(size=14))
fig.add_shape(type="rect", xref='x domain', yref='y domain', x0=0.642, y0=-.24, x1=0.658, y1=-.28, line=dict(color="#FF218C", width=2), fillcolor="#FF218C", row=7, col=2)
fig.add_annotation(xref='x domain', yref='y domain', x=0.736, y=-.31, text='RAM', showarrow=False, row=7, col=2, font=dict(size=14))
