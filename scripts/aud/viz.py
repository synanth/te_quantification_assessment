...
vsd_loc = base_loc + "aud_vsd.csv"
metadata_loc = "/lustre/research/dawli/stexocaelum/nac_aud/metadata.csv"
viz_loc = base_loc + "aud_viz.png"
raw_loc = "/lustre/research/dawli/stexocaelum/nac_aud/counts.csv"
...
tes_for_pca = deg_df.sort_values(by="padj")[:10].index.tolist()
...
fig = make_subplots(rows=2, cols=4, 
    subplot_titles=("PCA of top 10 TEs based on adjusted <i>P</i>-value", "Volcano plot"),
...
## volcano ##
degs_up = deg_df[(deg_df["pvalue"] <= .05) & (deg_df["log2FoldChange"] >= 1)]
degs_down = deg_df[(deg_df["pvalue"] <= .05) & (deg_df["log2FoldChange"] <= -1)]
non_degs = deg_df[(deg_df["pvalue"] > .05) & (deg_df["log2FoldChange"] < 1) & (deg_df["log2FoldChange"] > -1)]
...
fig.add_trace(go.Scattergl(x=degs_down["log2FoldChange"], y = -1 * np.log10(degs_down["pvalue"]), mode='markers', marker_color=colors[3], opacity=0.5, marker_size=10), row=1, col=3)
fig.add_trace(go.Scattergl(x=degs_up["log2FoldChange"], y = -1 * np.log10(degs_up["pvalue"]), mode='markers', marker_color=colors[1], opacity=0.5, marker_size=10), row=1, col=3)
fig.add_vline(x=1, line_dash="dash", line_color="#777777", row=1, col=3)
fig.add_vline(x=-1, line_dash="dash", line_color="#777777", row=1, col=3)
fig.add_hline(y=np.log10(.05)*-1, line_dash="dash", line_color="#777777", row=1, col=3)
...
## legend ##
fig.add_shape(type="rect", xref="paper", yref="paper", x0=.13, x1=.14, y0=1.05, y1=1.07, line=dict(color=colors[0]), fillcolor=colors[0])
fig.add_shape(type="rect", xref="paper", yref="paper", x0=.28, x1=.29, y0=1.05, y1=1.07, line=dict(color=colors[2]), fillcolor=colors[2])
fig.add_shape(type="rect", xref="paper", yref="paper", x0=.63, x1=.64, y0=1.05, y1=1.07, line=dict(color=colors[1]), fillcolor=colors[1])
fig.add_shape(type="rect", xref="paper", yref="paper", x0=.84, x1=.85, y0=1.05, y1=1.07, line=dict(color=colors[3]), fillcolor=colors[3])

fig.add_annotation(text="Alcohol Use Disorder", xref="paper", yref="paper", x=.141, y= 1.08, showarrow=False, font=dict(size=14))
fig.add_annotation(text="Control", xref="paper", yref="paper", x=.29, y= 1.08, showarrow=False, font=dict(size=14))
fig.add_annotation(text="Upregulated", xref="paper", yref="paper", x=.715, y= 1.08, showarrow=False, font=dict(size=14))
fig.add_annotation(text="Downregulated", xref="paper", yref="paper", x=.94, y= 1.08, showarrow=False, font=dict(size=14))
...
fig.write_image(height=600, width=1200, file=viz_loc, format="png")
