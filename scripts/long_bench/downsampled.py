...
out_loc = base_loc + "downsampled.csv"
n_tes = [100, 1000, 10000]
    ...
    for n in n_tes:
        for method in methods:
            for i in range(0, 10):
                ...
                lr_counts = dict(random.sample(list(lr_counts.items()), min(n, len(lr_counts))))
                ...
                with open(gtf_loc, "r") as f:
                    for line in f.readlines():
                        buff = line.strip().split()
                        name = buff[11][1:-2]
                        if name not in lr_counts or lr_counts.get(name, 0) == 0:
                            continue
                        lrs += [math.log2(lr_counts.get(name,0)+1)]
                        srs += [math.log2(sr_counts.get(name,0)+1)]
         
                df = pd.DataFrame({"lr" : lrs, "sr" : srs})
                spearman = str(df["sr"].corr(df["lr"], method="spearman"))
                pearson = str(df["sr"].corr(df["lr"], method="pearson"))
                mae = str(np.mean(np.abs(df["lr"] - df["sr"])).item())
                rmse = str(np.sqrt(np.mean((df["lr"] - df["sr"])**2)).item())
                out_csv += [[lr, sr, str(n), method, spearman, pearson, mae, rmse]]
print(out_csv)
