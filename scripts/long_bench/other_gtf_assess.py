...
out_csv = [["long_srr", "short_srr", "gtf", "method", "spearman", "pearson", "mae", "rmse"]]
out_loc = base_loc + "other_gtf.csv"
...
for lr, sr in lr_to_sr.items():
    ...
    for method in methods:
        for gtf in gtfs:
            ...
            srs = []
            tp, fp, fn = 0, 0, 0
            gtf_loc = "/lustre/work/stexocae/li_lab/refs/te_annotations/simulation/chm13/" + gtf + ".gtf"  
            long_read_loc = base_loc + lr + "/fc_" + gtf + ".out"
            if method == "te-saem":
                short_read_loc = base_loc + sr + "/" + method + "/" + gtf + "/counts.out"
            else:
            ...
            out_csv += [[lr, sr, gtf, method, spearman, pearson, mae, rmse]]
            print(out_csv[-1])
